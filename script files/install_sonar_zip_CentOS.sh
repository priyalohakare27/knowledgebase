...
#wget --directory-prefix=/opt http://dist.sonar.codehaus.org/sonarqube-$SONAR_VERSION.zip
wget --directory-prefix=/opt http://172.27.59.62/sonar/sonarqube-$SONAR_VERSION.zip
...
unzip -d /opt /opt/sonarqube-$SONAR_VERSION.zip
...
if [ "${MACHINE_TYPE}" = "64" ]; then
  ...
  /opt/sonar/bin/linux-x86-32/sonar.sh start
fi
...
wget --directory-prefix=/opt http://172.27.59.62/sonar/sonar-runner-dist-2.4.zip
